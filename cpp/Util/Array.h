...
public:
	...
	//insert a the data between the iterators @srcStart and @srcEnd into @this at position @p
	template <typename Iter2> inline void insertSlice(int p, Iter2 srcStart, Iter2 srcEnd)
	{
		...
		if ( p >= sz )
		{
			//existing elements dont need to be moved; add the elements to the end
			int newSize = p + n;
			increaseCapacity( newSize );

			//put blank elements up to @p
			constructArray( data + sz, p - sz );
			//fill the rest with the slice of @src
			constructArray( data + p, n, srcStart );
		...
		else
		{
			if ( n > 0 )
			{
				...
				else
				{
					...
					constructArray( data + sz, insertEnd - sz, srcStart + startToSz );
				}
	...
	//insert the slice of @src from @srcStart to @srcEnd into @this at position @p
	template <typename A2> inline void insertSlice(int p, const Array<T,A2> &src, int srcStart, int srcEnd)
	{
		insertSlice( p, src.begin() + srcStart, src.begin() + srcEnd );
	}

	//insert the contents of @src into @this at position @p
	template <typename A2> inline void insertSlice(int p, const Array<T,A2> &src)
	{
		insertSlice( p, src.begin(), src.end() );
	}


	//insert @element at position of iterator @pos
	template <typename Iter2> inline void insertSlice(iterator pos, Iter2 srcStart, Iter2 srcEnd)
	{
		insertSlice( pos - data, srcStart, srcEnd );
	}


	//remove the elements in the range start->end
	inline void removeSlice(int start, int end)
	{
		...
		if ( start < sz )
		{
			if ( end  >=  sz )
			{
				//if end >= sz, then only elements at the end are removed, so
	...
	//remove a range specified by iterators
	inline void removeSlice(iterator rangeStart, iterator rangeEnd)
	{
		removeSlice( rangeStart - data, rangeEnd - data );
	}
	...
	//add elements from range specified by @srcStart, and @srcEnd
	template <typename Iter2> inline void extend(Iter2 srcStart, Iter2 srcEnd)
	{
		int n = srcEnd - srcStart;
		reserveExtra( n );
		constructArray( data + sz, n, srcStart );

		sz += n;
	}

	//add elements from another array
	template <typename A2> inline void extend(const Array<T,A2> &src, int srcStart, int srcEnd)
	{
		extend( src.begin() + srcStart, src.begin() + srcEnd );
	}

	//add elements from another array
	template <typename A2> inline void extend(const Array<T,A2> &src)
	{
		extend( src.begin(), src.end() );
	}


	//copy elements from another array, wrapping over the end of the
	//source array as many times as necessary
	template <typename A2> inline void wrappedExtend(const Array<T,A2> &from, int start, int n)
	{
		//reserve space
		reserveExtra( n );
	...
	//Replace contents with elements from another array
	template <typename A2> inline void replaceContentsWith(const Array<T,A2> &from)
	{
		reserve( from.size() );
	...
	inline void repeat(int start, int end)
	{
		extend( begin() + start, begin() + end );
	}


	//reverse order of a subset of the elements
	inline void reverse(int start, int end)
	{
	...
	//set slice from iterator range
	template <typename Iter2> inline void setSlice(int dstStart, int dstEnd, Iter2 srcStart, Iter2 srcEnd)
	{
		int dstN = dstEnd - dstStart;
		int srcN = srcEnd - srcStart;

		// Copy elements from src to dst
		int numToCopy = std::min( srcN, dstN );
		for (int i = 0; i < numToCopy; i++)
		{
			data[dstStart+i] = srcStart[i];
		}

		if ( srcN > dstN )
		{
			// Need additional space in @this; insert
			insert( dstStart + dstN, srcStart + dstN, srcEnd );
		}
		else if ( srcN < dstN )
		{
			removeSlice( dstStart + srcN, dstEnd );
		}
	}

	//set slice
	template <typename A2> inline void setSlice(int dstStart, int dstEnd, const Array<T, A2> &src, int srcStart, int srcEnd)
	{
		setSlice( dstStart, dstEnd, src.begin() + srcStart, src.begin() + srcEnd );
	}
