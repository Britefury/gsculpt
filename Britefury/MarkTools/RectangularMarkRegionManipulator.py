...
from Britefury.Graphics.Graphics import Region3d, RectangleRegion2d, ProjectedRegion3d
...
class RectangularMarkRegionManipulator (MarkRegionManipulator, ViewportEventHandler, Viewport3dPainter):
	...
	def __init__(self):
		super( RectangularMarkRegionManipulator, self ).__init__()
	...
	def manipAttachToView(self, view):
		super( RectangularMarkRegionManipulator, self ).manipAttachToView( view )
	...
	def manipDetachFromView(self):
		...
		self._view.postRedraw()
		super( RectangularMarkRegionManipulator, self ).manipDetachFromView()



	def evPrimaryButtonDown(self, viewport):
		super( RectangularMarkRegionManipulator, self ).evPrimaryButtonDown( viewport )
	...
	def evMotion(self, viewport, point):
		super( RectangularMarkRegionManipulator, self ).evMotion( viewport, point )
		...
		if self._bButtonDown:
			...
			if self._regionCell is not None:
				cam = viewport.camera
				region2d = RectangleRegion2d( self._p1, self._p2 )
	...
	def evPrimaryButtonUp(self, viewport):
		super( RectangularMarkRegionManipulator, self ).evPrimaryButtonUp( viewport )
	...
	def evPaint3d(self, viewport, paintLayer):
		...
		if paintLayer == PaintLayer.PAINTLAYER_OVERLAY_BLEND_NODEPTH:
			if viewport is self._regionViewport:
				...
				box = BBox2( self._p1, self._p2 )
				ModelDraw.drawMarkBox( box )
...
class RectangularMarkRegionFieldManipulator (GSProcedureToolFieldManipulator):
	def __init__(self, predicateField, regionField, backfaceCullField, bEnabledField):
		super( RectangularMarkRegionFieldManipulator, self ).__init__()
	...
	def createElement(self, sheetEditContext):
		return RectangularMarkRegionManipulator()
