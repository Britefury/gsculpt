...
from Britefury.Mesh.Mesh import MeshBevelEdgesAdjuster, GSProductMesh



class ProcMeshBevelEdges (GSProcedure):
	description = _( 'Bevel edges' )
	...
	def procedureInvoke(self, inputObject, errorHandler):
		adjuster = MeshBevelEdgesAdjuster()
		inputObject.bevelMarkedEdges( adjuster, self.bUseExistingTexturing, self.bMarkBoundaryEdges, self.bNSharpBoundaryEdges )
		adjuster.setParameters( self.bevel )
		return inputObject



class BevelEdgesAdjustableInvoker (GSProcedureAdjustableInvoker):
	functionClass = ProcMeshBevelEdges

	def main(self):
		self.mesh = deepcopy( self._inputObject )
		self.adjuster = MeshBevelEdgesAdjuster()
		self.mesh.bevelMarkedEdges( self.adjuster, self._proc.bUseExistingTexturing, self._proc.bMarkBoundaryEdges, self._proc.bNSharpBoundaryEdges )
...
class ToolMeshBevelEdges (GSProcedureTool):
	sheet = SheetRefField( ProcMeshBevelEdges )

	functionClass = ProcMeshBevelEdges
	adjustableInvoker = BevelEdgesAdjustableInvoker
	pageTitle = _( 'Bevel' )
	titleText = _( 'Bevel Edges' )
