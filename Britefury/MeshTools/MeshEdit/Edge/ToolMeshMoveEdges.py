...
class ProcMeshMoveEdges (ProcMeshEditMove):
	description = _( 'Move edges' )

	def procedureInvoke(self, inputObject, errorHandler):
		transformation = MeshTransformation()
		inputObject.initEdgeTransformation( transformation, False )
		transformation.translate( self.translation )
		return inputObject



class MoveEdgesAdjustableInvoker (GSProcedureAdjustableInvoker):
	functionClass = ProcMeshMoveEdges

	def main(self):
		self.mesh = deepcopy( self._inputObject )
		self.transformation = MeshTransformation()
		self.mesh.initEdgeTransformation( self.transformation, False )
...
class ToolMeshMoveEdges (ToolMeshEditMove):
	sheet = SheetRefField( ProcMeshMoveEdges )

	functionClass = ProcMeshMoveEdges
	adjustableInvoker = MoveEdgesAdjustableInvoker
	pageTitle = _( 'Move' )
	titleText = _( 'Move Edges' )
