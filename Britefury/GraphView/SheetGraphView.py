...
class _SheetGraphViewGraphLinkList (_SheetGraphViewItemListHelper):
	...
	def attachGraph(self, graph):
		...
		self._graph.nodeAddedSignal.connect( self._p_onNodeAppended )
		self._graph.removeNodeSignal.connect( self._p_onRemoveNode )

		for node in self._graph.nodes:
			self._p_onNodeAppended( graph, node )


	def detachGraph(self):
		self._graph.nodeAddedSignal.disconnect( self._p_onNodeAppended )
		self._graph.removeNodeSignal.disconnect( self._p_onRemoveNode )

		for node in self._graph.nodes:
			self._p_onRemoveNode( self._graph, node )
	...
	def _p_onNodeAppended(self, graph, node):
	...
	def _p_onRemoveNode(self, graph, node):
		# TODO: handle changing sink list
		for sink in node.sinks:
