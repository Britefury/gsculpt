...
def _readVertex(l):
	...
	if sre.search("facet", l) or sre.search("outer", l) or sre.search("endloop", l) or sre.search("endfacet", l):
		return None

	vertex = sre.search("vertex\s*([\d\-+\.EeDd]+)\s*([\d\-+\.EeDd]+)\s*([\d\-+\.EeDd]+)", l)
...
#               print "file is ascii: ", self.isascii
	...
	def IsAscii(self, fl):
		l = fl.readline(1024)
		isascii = l[:5] == "solid" and (len(l) == 5 or (sre.search("[^A-Za-z0-9\,\.\/\;\:\'\"\+\-\s\r\n]", l[6:]) == None)) # some files say 'solid' but are binary files, we try to find a non alphanumerical character in the rest to the first line
		fl.seek(0)
		return isascii
